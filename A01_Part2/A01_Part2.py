...
def my_main(input_folder, output_file, bike_id):
    ...
    for files in list_of_files:
        ...
        while True:
            ...
            if line[11] == str(bike_id):
                current_start_station = line[4]
                current_stop_station = line[8]

                # The != -1 condition is to handle the initial row as it will not have a previous station id due to it
                # being the first trip and having no records before it.
                if (previous_stop_station != -1 and current_start_station != previous_stop_station):
                    text_file.write("By_Truck" + '\t' + "(" + previous_station_log_time + ", " + previous_stop_station + ", "
                               + line[0] + ", " + current_start_station + ")" + "\n")

                    #count += 1

                previous_stop_station = current_stop_station
                previous_station_log_time = line[1]
